...
import exportHasura from './export/hasura';
...
export async function worker(folder, schema, config) {
	try {
		...
		xmlFiles.forEach(async (xmlToSave, fileName) => {
			await writeFile(
				'./' + folder + '/' + constants.confAttrXmlSnippetsFolder + '/' + fileName,
				xmlToSave,
				'utf8'
			);
		});
		...
		// Usermanegement -----------------------------------------------------------------------
		const { userArray, groups } = await exportUserManagement(
			client,
			groupConfigAttrs,
			userConfigAttrs
		);
		...
		const { cidsClasses, attributes } = await exportClasses(client);
		console.log('writing classes.json');
		await writeFile(
			'./' + folder + '/classes.json',
			stringify(cidsClasses, {
				maxLength: 100
			}),
			'utf8'
		);
		...
		// classes and attributes -----------------------------------------------------------------------
		const { hasuraMeta } = await exportHasura(client, cidsClasses, attributes);
		console.log('writing hasura.json');
		await writeFile(
			'./' + folder + '/hasura.json',
			stringify(hasuraMeta, {
				maxLength: 100
			}),
			'utf8'
		);
		...
		await writeFile(
			'./' + folder + '/classPerms.json',
			stringify(cPermByTable, {
				maxLength: 100
			}),
			'utf8'
		);
		...
		await writeFile(
			'./' + folder + '/normalizedClassPerms.json',
			stringify(normalizedClassPermResult, {
				maxLength: 100
			}),
			'utf8'
		);
		...
		//attr permissions -----------------------------------------------------------------------
		const { aPermByTable, normalizedAttrPermResult } = await exportAttrPermissions(
			client,
			attributes,
			classReadPerms,
			classWritePerms
		...
		await writeFile(
			'./' + folder + '/attrPerms.json',
			stringify(aPermByTable, {
				maxLength: 100
			}),
			'utf8'
		);
		...
		await writeFile(
			'./' + folder + '/normalizedAttrPerms.json',
			stringify(normalizedAttrPermResult, {
				maxLength: 100
			}),
			'utf8'
		);
		...
		await writeFile(
			'./' + folder + '/dynchildhelpers.json',
			stringify(dynchildhelpers, {
				maxLength: 80
			}),
			'utf8'
		);
		...
		structureSqlDocuments.forEach(async (value, key) => {
			await writeFile(
				'./' + folder + '/' + constants.structureDynamicChildrenFolder + '/' + key,
				value,
				'utf8'
			);
		});
		...
		helperSqlDocuments.forEach(async (value, key) => {
			await writeFile(
				'./' + folder + '/' + constants.structureHelperStatementsFolder + '/' + key,
				value,
				'utf8'
			);
		});
